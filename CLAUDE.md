...
| `usage_guard.py` | (v3.6.0) Server-side daily run limits that survive a new session (unlike `run_count`): a global daily cap + a best-effort per-client cap keyed on a date-salted SHA-256 of the client IP (raw IP never stored; skipped for private/loopback addresses so users behind one proxy can't lock each other out). Counters live in Pinecone's `app-metrics` namespace. Fails open on any store error. Limits overridable via `QAI_GLOBAL_DAILY_RUN_LIMIT` / `QAI_CLIENT_DAILY_RUN_LIMIT` (defaults 100 / 10). `app.py`'s `_enforce_daily_quota()` calls it at all 3 run-consuming sites |
| `executive_readout.py` | (v3.6.0) Deterministic, dependency-free "so what?" summary shown above `render_strategy()`'s 4 tabs: `build_readout(risk_rows, effort_data)` → overall risk + breakdown, effort range, capacity vs. expected effort, confidence, top-3 risks by priority. No LLM call, never invents actions; rendered by `components.executive_readout_html()` |
...
- **`LLMClient._chat_stream()` may only fall back to OpenRouter before the first chunk is yielded** — OpenRouter restarts the document from scratch, so a mid-stream fallback duplicates the already-shown opening; after the first chunk it raises `QAIConnectionError` instead.
...
- **Mistral's Free plan 429s `mistral-small`/`medium`/`magistral` (code 1300) at near-zero usage while serving the Ministral family** — a 429 with a nearly empty Usage page is a plan/tier block, not real rate limiting; probe several models before blaming the key or quota. It began ~2026-09-03 and went unnoticed for ~3 weeks because `LLMClient` silently fell back to OpenRouter (and the live Mistral contract test passed through that fallback until v3.6.2); check OpenRouter's Activity page for a spike in fallback traffic to date an outage — local logs only go back to 2026-09-22.
