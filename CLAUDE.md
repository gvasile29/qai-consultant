...
- **v3.2** CI quality gates completion — Phase 4 of `docs/superpowers/plans/2026-07-21-ci-quality-gates-implementation.md` (Phases 1-3 already live in `.github/workflows/ci.yml`, see the CI section above): a separate, always-green-by-construction `live-contract-tests.yml` workflow (`schedule` cron + `workflow_dispatch` only, never `push`/`pull_request`) running `tests/test_live_contracts.py` — real Pinecone upsert/fetch/delete roundtrip, real Mistral completion, real OpenRouter completion — each SKIPping (not failing) without its required secret(s), same pattern as the existing live-LLM tests in `test_risk_analyzer.py`. Design spec: `docs/superpowers/specs/2026-07-21-ci-live-contract-tests-design.md`.
