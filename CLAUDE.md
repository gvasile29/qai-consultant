...
| `visit_counter.py` | (v3.1.1) `get_and_increment_visit_count()` — Streamlit-only visit counter persisted in Pinecone's `app-metrics` namespace (isolated from the RAG `knowledge-base` namespace), fixed vector ID `visit_counter`, `{"count": <int>}` metadata; fetch→increment→upsert, wrapped in `try/except Exception: return None` so it never crashes the app |
