...
- **v3.1.4** ✅ Added the `mcp-name: io.github.gvasile29/qai-consultant-mcp` marker (an HTML comment at the top of `README_MCP.md`) — a prerequisite for the official Anthropic MCP registry, which validates this marker against the **live PyPI JSON API** response for the package, not the GitHub repo's README. This is why a version bump was required purely to add one metadata line: PyPI never allows re-uploading an already-published version's files/metadata, so 3.1.3's published README couldn't be edited in place. No functional change.
