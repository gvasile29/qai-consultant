...
- [x] **Glama** (glama.ai/mcp) — submitted via the "Add Server" form (Name/Description/GitHub URL only; confirmed no login needed for this initial step — "Public submissions are reviewed before becoming publicly visible"). "Your server has been submitted for review" confirmed. Approved 2026-07-28; listing claimed via GitHub OAuth (Gabi's own account). **Done.** Glama's admin Dockerfile page turned out to be a structured builder, not a raw-paste field: it clones the repo directly (`git clone` + `git checkout <pinned SHA>`) and runs a JSON "Build steps" array via `uv sync`, not a `pip install qai-consultant-mcp==X` flow like the scratch Dockerfile below — so that snippet (kept for reference) was never actually pasted anywhere. The first build test (2026-07-27, commit `59f5065`) failed with `MCP error -32001: Request timed out` — the exact cold-start failure mode documented in `CLAUDE.md`'s "MCP server: embedding-model warmup must happen before `mcp.run()`" gotcha: Glama's build only ran `uv sync`, so the container's first real run had to embed the whole KB live, blowing past `mcp-proxy`'s 60s ping timeout. Fixed by adding a second Build step that bakes the KB index at build time instead of runtime: `uv run python -c "from local_index import LocalIndex; LocalIndex().search('warmup', k=1)"`. Re-tested 2026-07-28 (test `019fa715-e046-7d36-9cec-876f833ff8de`): **success**, 1m49s, real stdio MCP handshake confirmed (`initialize`, all 4 tools + 4 prompts listed correctly). Release `3.1.4` created and published from that test the same day — live on the listing now.

  Scratch Dockerfile prepared earlier for a plain `pip install`-style host (not what Glama actually uses, kept here in case another directory wants a standalone Dockerfile):

  ```dockerfile
  FROM python:3.12-slim
  COPY --from=ghcr.io/astral-sh/uv:latest /uv /uvx /bin/
  RUN uv pip install --system qai-consultant-mcp==3.1.4
...
**Acceptance:** submissions prepared (and, where Gabi approves, submitted) to at least the Anthropic registry + one awesome-list PR + two of the three third-party directories. **Met** — Anthropic registry live, awesome-mcp-servers PR open, mcp.so submitted, Glama live with a published release and passing build test (Smithery dropped by decision, not attempted). Phase 5 is effectively done pending only the awesome-mcp-servers PR merge (its bot was asking for the Glama score badge, now unblocked since Glama has a successful build test and release).
...
| Glama | 2026-07-23 | Live, release published | Add Server form (Name/Description/GitHub URL); no login needed for that step. Approved and listing claimed via GitHub OAuth 2026-07-28. Fixed a build-time KB-warmup cold-start failure (same root cause as the `CLAUDE.md` MCP warmup gotcha) by adding a build step to Glama's own build spec; re-tested successfully (1m49s, full stdio handshake); release `3.1.4` created and published 2026-07-28 (see Phase 5 above). |
...
- [x] **Glama badges (2026-07-28):** added the card + score badges Glama generates once a server has a passing build/release (`badges/card.svg`, `badges/score.svg`, both linking to the listing) to both `README.md` (under the "MCP Server" section heading) and `README_MCP.md` (under the title, above the demo GIF) — the same placement pattern as the demo GIF above.
