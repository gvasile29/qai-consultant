...
**Resolution:** gate one-shot entrance sequences behind a `st.session_state` "seen" flag, the same idiom the codebase already uses for `mcp_announcement_seen` (`app.py`) and the visit counter — e.g. `dialogue_intro_animated` / `review_intro_animated`, set `True` after first render of that screen in the session, checked before including the entrance-animation CSS classes on subsequent reruns of the same screen. This is a session-scoped, per-screen flag, not a single global one, so returning to `render_dialogue()` after visiting `render_review()` does not require a fresh entrance every time either (each screen animates once per session, not once per app-visit).
...
- **Question cards (`.ledger-card`):** add a `:hover` rule (border + lift, matching the pattern already established for Phase 1's `.pom-card`, but scoped to `.ledger-card` itself this time since this phase owns that class). No per-card entrance animation — with 11 cards re-rendering on every template-dropdown rerun, a staggered fade-in would replay distractingly often; a static-but-responsive card serves the "calibrated instrument" feeling better here than repeated motion would.
